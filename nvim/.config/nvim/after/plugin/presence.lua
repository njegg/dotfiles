...
require("presence").setup({
    ...
    neovim_image_text   = "kako?",                    -- Text displayed when hovered over the Neovim image
    ...
    buttons             = {                          -- Configure Rich Presence button(s), either a boolean to enable/disable, a static table (`{{ label = "<label>", url = "<url>" }, ...}`, or a function(buffer: string, repo_url: string|nil): table)
        { label = "My Onlyfans", url = "https://blogs.mtdv.me/njegg" },
    },
    ...
    editing_text        = (function(filename)
        ...
        if extension then
            return "Editing a ." .. extension .. " file"
        else
            return "kako?"
        end
    ...
    workspace_text      = "yes",            -- Format string rendered when in a git repository (either string or function(project_name: string|nil, filename: string): string)
    line_number_text    = "Errors found: %s",        -- Format string rendered when `enable_line_number` is set to true (either string or function(line_number: number, line_count: number): string)
})
